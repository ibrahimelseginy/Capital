        ...
        <div class="grid-3">
          ...
          <a href="#/partner/${p.enName}" class="card card-hover partner-item" data-category="${p.cat}" data-name="${p.name}">
            <div style="height:120px;background:var(--bg-secondary);display:flex;align-items:center;justify-content:center;padding:var(--space-4)">
              <img src="assets/images/${p.img}.png" class="partner-logo-img" alt="${p.name}" style="max-height:60px">
            </div>
            <div class="card-body">
      ...
      <div class="grid-4 reveal">
        ...
        <a href="#/partner/${p.enName}" class="card card-hover partner-item" data-category="${p.cat}" data-name="${p.name}">
          <div style="height:100px;background:var(--bg-secondary);display:flex;align-items:center;justify-content:center;padding:var(--space-4)">
            <img src="assets/images/${p.img}.png" class="partner-logo-img" alt="${p.name}" style="max-height:40px">
          </div>
...
// EVENTS DATA
// ════════════════════════════════════════
const getEvents = (isAr) => isAr ? [
  { id: 'demo-day-2026', title:'يوم عروض المشاريع 2026', date:'يوليو 15', loc:'الرياض، السعودية', cat:'يوم عروض', status:'التسجيل مفتوح', type:'حضوري', img:'event-demo-day', desc: 'شاهد أحدث مشاريعنا وهي تُعرض أمام نخبة مختارة من المستثمرين، الشركاء، وقادة الصناعة. ست شركات، اثنتا عشرة دقيقة لكل منها، إمكانيات لا محدودة.' },
  { id: 'investor-briefing', title:'لقاء المستثمرين: تحديث الربع الثاني', date:'يوليو 28', loc:'عبر الإنترنت', cat:'ندوة', status:'دعوة فقط', type:'عن بُعد', img:'event-investor-briefing', desc: 'تحديث حصري لمستثمرينا حول أداء الربع الثاني وأبرز الإنجازات.' },
  { id: 'product-market-fit', title:'ورشة عمل ملاءمة المنتج للسوق', date:'أغسطس 10', loc:'دبي، الإمارات', cat:'ورشة عمل', status:'ممتلئ', type:'حضوري', img:'event-workshop', desc: 'ورشة عمل مكثفة لرواد الأعمال حول تحقيق ملاءمة المنتج للسوق بفاعلية.' },
  { id: 'mena-tech-summit', title:'قمة التقنية للشرق الأوسط', date:'سبتمبر 05', loc:'الرياض، السعودية', cat:'مؤتمر', status:'قريباً', type:'حضوري', img:'blog-building-scale', desc: 'أكبر تجمع لقادة التقنية في الشرق الأوسط.' },
  { id: 'founders-networking', title:'غداء عمل رواد الأعمال', date:'سبتمبر 12', loc:'الرياض، السعودية', cat:'تعارف', status:'التسجيل مفتوح', type:'حضوري', img:'blog-mena-market', desc: 'فرصة رائعة للتواصل مع نخبة من رواد الأعمال والمستثمرين.' },
] : [
  { id: 'demo-day-2026', title:'Venture Demo Day 2026', date:'Jul 15', loc:'Riyadh, KSA', cat:'Demo Day', status:'Registration Open', type:'In-Person', img:'event-demo-day', desc: 'Watch our latest ventures present to a curated audience of investors, partners, and industry leaders. Six companies, twelve minutes each, unlimited potential.' },
  { id: 'investor-briefing', title:'Investor Briefing: Q2 Update', date:'Jul 28', loc:'Online', cat:'Webinar', status:'Invitation Only', type:'Remote', img:'event-investor-briefing', desc: 'Exclusive update for our investors on Q2 performance and key milestones.' },
  { id: 'product-market-fit', title:'Product-Market Fit Workshop', date:'Aug 10', loc:'Dubai, UAE', cat:'Workshop', status:'At Capacity', type:'In-Person', img:'event-workshop', desc: 'Intensive workshop for entrepreneurs on achieving product-market fit effectively.' },
  { id: 'mena-tech-summit', title:'MENA Tech Summit 2026', date:'Sep 05', loc:'Riyadh, KSA', cat:'Conference', status:'Coming Soon', type:'In-Person', img:'blog-building-scale', desc: 'The largest gathering of tech leaders in the Middle East.' },
  { id: 'founders-networking', title:'Founders Networking Lunch', date:'Sep 12', loc:'Riyadh, KSA', cat:'Networking', status:'Registration Open', type:'In-Person', img:'blog-mena-market', desc: 'A great opportunity to network with top entrepreneurs and investors.' },
];
...
function eventsPage() {
  ...
  <section class="section" style="padding-top:calc(var(--header-height) + var(--space-8))">
    <div class="container-content">
      ...
      <div class="grid-3 reveal">
        ${events.map((e, i) => `
        <a href="#/event/${e.id}" class="event-card" data-cat="${e.cat}" data-time="${e.status.includes('Past') || e.status.includes('السابقة') ? 'past' : 'upcoming'}">
...
function eventDetailPage(params) {
  ...
  const evCat = event.cat;
  const evStatus = event.status;
  const evType = event.type;
  const evDesc = event.desc;
    
  const dateStr = event.date;
  const locStr = event.loc;
  ...
  <section class="section" style="padding-top:calc(var(--header-height) + var(--space-8))">
    <div class="container-content">
      ...
      <div class="grid-12" style="gap:var(--space-10)">
        <div style="grid-column:span 8">
          <div style="height:360px;background:url('${event.img.includes('assets/images/') ? event.img : 'assets/images/'+event.img+'.png'}') center/cover;border-radius:var(--radius-xl);margin-bottom:var(--space-8)"></div>
