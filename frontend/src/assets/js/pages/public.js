...
// PARTNER DETAIL
// ════════════════════════════════════════
export function partnerDetailPage(params) {
...
// ════════════════════════════════════════
export function investorsPublicPage() {
  const isAr = LangManager.currentLang === 'ar';
...
const getArticles = (isAr) => isAr ? [
  { id: 'future-of-venture-studios', title: 'لماذا تعتبر استوديوهات المشاريع هي مستقبل بناء الشركات', cat: 'بناء المشاريع', author: 'أحمد الراشد', date: '5 يونيو 2026', time: '8', img: 'blog-venture-studios', tags: ['#تقنية', '#استوديوهات_مشاريع', '#بناء_شركات'], content: '<p>النموذج التقليدي للشركات الناشئة لم يعد فعالاً. توفر استوديوهات المشاريع رأس المال والفريق والبنية التحتية التشغيلية التي يحتاجها المؤسسون للنجاح من اليوم الأول.</p><p>هنا نقوم ببناء المستقبل. ##استثمار ##مستقبل</p>', media: [ { type: 'image', url: 'images/blog-venture-studios.png' }, { type: 'youtube', url: 'https://www.youtube.com/embed/dQw4w9WgXcQ' } ] },
  { id: 'investment-returns-data', title: 'فهم عوائد استوديوهات المشاريع: تحليل مبني على البيانات', cat: 'الاستثمار', author: 'سارة التميمي', date: '28 مايو 2026', time: '6', img: 'blog-investment-returns', tags: ['#استثمار', '#بيانات', '#عائد'], content: '<p>تحليل عميق لكيفية تحقيق عوائد استثنائية من خلال منهجية الاستوديو.</p>', media: [ { type: 'gallery', urls: ['images/blog-investment-returns.png', 'images/blog-building-scale.png'] }, { type: 'file', url: '#', name: 'تقرير العوائد.pdf' } ] },
      ...
      <div class="grid-3 reveal">
        ...
        <div class="blog-item-wrapper" data-cat="${a.cat}" data-tags="${(a.tags||[]).join(',')}">
          <a href="#/blog/${a.id}" class="blog-card-small" style="height:100%">
            <div class="blog-card-small-image" style="background-image:url('${a.img.includes('images/') ? a.img : 'images/'+a.img+'.png'}');background-size:cover;background-position:center;"></div>
    ...
    <div class="container-content" style="position: relative; z-index: 1;">
      ...
      <div class="grid-3 reveal">
        ...
        <a href="#/event/${e.title.toLowerCase().replace(/[^a-z0-9؀-ۿ]+/g,'-')}" class="event-card" data-cat="${e.cat}" data-time="${e.status.includes('Past') || e.status.includes('السابقة') ? 'past' : 'upcoming'}">
          <div class="event-card-image" style="background:url('images/${e.img}.png') center/cover;position:relative">
