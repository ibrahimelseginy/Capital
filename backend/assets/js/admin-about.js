(function(){
  'use strict';
  const manager=document.querySelector('[data-admin-about-manager]');
  if(!manager)return;
  const allowed=['create_about_item','update_about_item','delete_about_item'];
  manager.querySelectorAll('[data-admin-about-form]').forEach(function(form){
    form.addEventListener('submit',function(event){
      const submitter=event.submitter;
      const hidden=form.querySelector('input[name="action"]');
      const action=submitter&&submitter.name==='action'?submitter.value:(hidden?hidden.value:'');
      if(!allowed.includes(action))return;
      event.preventDefault();
      if(action!=='delete_about_item'&&!form.reportValidity())return;
      const data=new FormData(form);data.set('action',action);
      const buttons=form.querySelectorAll('button[type="submit"],button:not([type])');
      buttons.forEach(function(button){button.disabled=true;});if(submitter)submitter.classList.add('is-loading');
      fetch('../api/admin-about.php',{method:'POST',body:data,headers:{Accept:'application/json'},credentials:'same-origin'})
