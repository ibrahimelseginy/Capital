...
@section('title', app()->getLocale() == 'ar' ? 'لوحة المستثمر' : 'Investor Dashboard')
...
<div class="card mb-8 reveal" style="padding:var(--space-8) var(--space-10);background:linear-gradient(135deg, #0f0f0f 0%, #1a1510 40%, #1a1714 60%, #0f0f0f 100%);border:1px solid rgba(255,90,0,0.15);position:relative;overflow:hidden;border-radius:var(--radius-xl)">
  ...
  <div style="position:relative;z-index:2">
    <div class="text-caption mb-3" style="color:var(--action-primary);font-weight:var(--weight-bold);text-transform:uppercase;letter-spacing:3px;font-size:11px">
        {{ app()->getLocale() == 'ar' ? 'لوحة المستثمر' : 'Investor Dashboard' }}
    </div>
    <h2 class="text-h2 mb-3" style="color:#fff;font-weight:var(--weight-bold);letter-spacing:-0.5px;line-height:1.2">
        {{ app()->getLocale() == 'ar' ? 'مرحباً بك، خالد' : 'Welcome back, Khalid' }} 👋
    </h2>
    <p class="text-body" style="color:rgba(255,255,255,0.55);max-width:500px;line-height:1.6">
        {{ app()->getLocale() == 'ar' ? 'إليك نظرة سريعة على أداء محفظتك والإجراءات المعلقة.' : 'Here\'s your portfolio performance and pending actions at a glance.' }}
    </p>
  </div>
</div>
...
<div class="grid-4 mb-8">
  <!-- Total Invested -->
  <div class="metric-card" onclick="window.location.href='{{ url('/dashboard/projects') }}'" style="padding:var(--space-6);border-radius:var(--radius-xl);border:1px solid var(--border-default);background:var(--bg-surface);position:relative;overflow:hidden;transition:all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);cursor:pointer">
  ...
  <div class="metric-card" onclick="window.location.href='{{ url('/dashboard/projects') }}'" style="padding:var(--space-6);border-radius:var(--radius-xl);border:1px solid var(--border-default);background:var(--bg-surface);position:relative;overflow:hidden;transition:all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);cursor:pointer">
    ...
    <div class="d-flex justify-between items-start" style="position:relative;z-index:2">
      <div>
        ...
        <div class="metric-card-value" style="font-size:2rem;line-height:1;letter-spacing:-1px">{{ $activeProjectsCount ?? 5 }}</div>
      </div>
  ...
  <!-- Pending NDAs -->
  <div class="metric-card" onclick="window.location.href='{{ url('/dashboard/ndas') }}'" style="padding:var(--space-6);border-radius:var(--radius-xl);border:1px solid var(--border-default);background:var(--bg-surface);position:relative;overflow:hidden;transition:all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);cursor:pointer">
...
<div class="grid-12" style="gap:var(--space-6)">
  <div style="grid-column:span 8">
    ...
    <div class="card mb-6" style="padding:var(--space-6);border-radius:var(--radius-xl)">
      <div class="d-flex justify-between items-center mb-5">
        ...
        <div class="d-flex gap-2" id="portfolio-overview-chips">
          <button class="chip active" style="border-radius:var(--radius-full)" onclick="document.querySelectorAll('#portfolio-overview-chips .chip').forEach(c=>c.classList.remove('active')); this.classList.add('active');">{{ app()->getLocale() == 'ar' ? 'سنة' : '1Y' }}</button>
          <button class="chip" style="border-radius:var(--radius-full)" onclick="document.querySelectorAll('#portfolio-overview-chips .chip').forEach(c=>c.classList.remove('active')); this.classList.add('active');">{{ app()->getLocale() == 'ar' ? 'الكل' : 'All' }}</button>
        </div>
      </div>
    ...
    <div class="card" style="padding:var(--space-6);border-radius:var(--radius-xl)">
      ...
      <!-- Activity 1 -->
      <div class="d-flex gap-4 py-3 activity-row" onclick="window.location.href='{{ url('/dashboard/reports') }}'" style="padding-inline:var(--space-3);border-radius:var(--radius-md);cursor:pointer;transition:all 0.2s">
        <div style="width:10px;height:10px;border-radius:50%;background:#3b82f6;margin-top:6px;flex-shrink:0;box-shadow:0 0 0 3px rgba(59,130,246,0.2)"></div>
      ...
      <!-- Activity 2 -->
      <div class="d-flex gap-4 py-3 activity-row" onclick="window.location.href='{{ url('/dashboard/projects') }}'" style="padding-inline:var(--space-3);border-radius:var(--radius-md);cursor:pointer;transition:all 0.2s">
        <div style="width:10px;height:10px;border-radius:50%;background:var(--color-success);margin-top:6px;flex-shrink:0;box-shadow:0 0 0 3px rgba(34,197,94,0.2)"></div>
      ...
      <!-- Activity 3 -->
      <div class="d-flex gap-4 py-3 activity-row" onclick="window.location.href='{{ url('/dashboard/exit-requests') }}'" style="padding-inline:var(--space-3);border-radius:var(--radius-md);cursor:pointer;transition:all 0.2s">
        <div style="width:10px;height:10px;border-radius:50%;background:var(--action-primary);margin-top:6px;flex-shrink:0;box-shadow:0 0 0 3px rgba(255,90,0,0.2)"></div>
      ...
      <!-- Activity 4 -->
      <div class="d-flex gap-4 py-3 activity-row" onclick="window.location.href='{{ url('/dashboard/ndas') }}'" style="padding-inline:var(--space-3);border-radius:var(--radius-md);cursor:pointer;transition:all 0.2s">
        <div style="width:10px;height:10px;border-radius:50%;background:var(--accent-gold);margin-top:6px;flex-shrink:0;box-shadow:0 0 0 3px rgba(198,161,91,0.2)"></div>
  ...
  <div style="grid-column:span 4">
    ...
    <!-- Upcoming -->
    <div class="card" style="margin-bottom:var(--space-6);padding:var(--space-6);border-radius:var(--radius-xl)">
      <h4 class="text-label mb-4" style="font-weight:var(--weight-bold)">{{ app()->getLocale() == 'ar' ? 'القادم' : 'Upcoming' }}</h4>
      <div class="d-flex flex-col gap-3">
        <div class="d-flex gap-3 items-center" onclick="window.location.href='{{ url('/dashboard/events') }}'" style="padding:var(--space-3);border-radius:var(--radius-lg);transition:all 0.2s;cursor:pointer">
