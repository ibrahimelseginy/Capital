...
if($method==='GET'){try{$rows=admin_rows('SELECT * FROM investor_page_items ORDER BY section_key,sort_order,created_at');foreach($rows as &$row){$row['sort_order']=(int)$row['sort_order'];$row['is_active']=(bool)$row['is_active'];}unset($row);echo json_encode(['ok'=>true,'data'=>$rows,'meta'=>['count'=>count($rows)]],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);}catch(Throwable $error){error_log('Admin investors page API read failed: '.$error->getMessage());http_response_code(503);echo json_encode(['ok'=>false,'message'=>'تعذر تحميل بيانات صفحة المستثمرين.'],JSON_UNESCAPED_UNICODE);}exit;}
...
$action=(string)($_POST['action']??'');if(!in_array($action,['create_investor_page_item','update_investor_page_item','delete_investor_page_item'],true)){http_response_code(400);echo json_encode(['ok'=>false,'message'=>'الإجراء المطلوب غير صحيح.'],JSON_UNESCAPED_UNICODE);exit;}
...
catch(Throwable $error){error_log('Admin investors page API write failed: '.$error->getMessage());http_response_code(500);echo json_encode(['ok'=>false,'message'=>'تعذر تنفيذ الطلب. تحقق من البيانات وحاول مرة أخرى.'],JSON_UNESCAPED_UNICODE);}
