...
try {
    ...
    if ($method==='GET') { echo json_encode(['ok'=>true,'data'=>about_read()['sections']],JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR); exit; }
    ...
    $result=about_save($key,$content,$revision,(string)$user['id']);
    echo json_encode(['ok'=>true,'message'=>'تم حفظ القسم ونشره على صفحة من نحن.']+$result,JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);
} catch (InvalidArgumentException $error) {
    http_response_code(422); echo json_encode(['ok'=>false,'message'=>$error->getMessage()],JSON_UNESCAPED_UNICODE);
} catch (Throwable $error) {
    error_log('About sections: '.$error->getMessage());
