...
try {
    // Do not authorize against the legacy file fallback when MySQL is down.
    home_db();
    auth_boot();
    ...
    if ($method==='GET') { echo json_encode(['ok'=>true,'data'=>home_read()],JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR); exit; }
    ...
    $next=home_save($key,$content,$revision,(string)$user['id']);
    echo json_encode(['ok'=>true,'revision'=>$next,'message'=>'تم حفظ القسم ونشره على الصفحة الرئيسية.'],JSON_UNESCAPED_UNICODE);
} catch (InvalidArgumentException $error) {
    http_response_code(422); echo json_encode(['ok'=>false,'message'=>$error->getMessage()],JSON_UNESCAPED_UNICODE);
} catch (Throwable $error) {
    error_log('Admin home: '.$error->getMessage());
