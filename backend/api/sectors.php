...
try {
    echo json_encode(sectors_public_payload(sectors_read()),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);
} catch (Throwable $error) {
    error_log('Sectors API failed: '.$error->getMessage());http_response_code(503);
    echo json_encode(['ok'=>false,'message'=>'تعذر تحميل صفحة القطاعات.'],JSON_UNESCAPED_UNICODE);
}
