...
try{
    news_page_db();auth_boot();$user=isset($_SESSION['user_id'])?auth_find_user_by_id((string)$_SESSION['user_id']):null;
    ...
    $method=$_SERVER['REQUEST_METHOD']??'GET';if($method==='GET'){echo json_encode(['ok'=>true,'data'=>news_page_read()],JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);exit;}
    ...
    $raw=$_POST['content']??null;if(!is_string($raw)||strlen($raw)>300000)throw new InvalidArgumentException('حجم المحتوى غير صالح.');
    ...
    $next=news_page_save($key,$content,$revision,(string)$user['id']);
    echo json_encode(['ok'=>true,'revision'=>$next,'message'=>'تم حفظ القسم ونشره على صفحة الأخبار.'],JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);
...
catch(Throwable $error){error_log('News page sections: '.$error->getMessage());$conflict=$error->getCode()===409;http_response_code($conflict?409:503);echo json_encode(['ok'=>false,'message'=>$conflict?$error->getMessage():'تعذر الحفظ. تحقق من تشغيل MySQL في MAMP. لم يتم نشر التغييرات.'],JSON_UNESCAPED_UNICODE);}
