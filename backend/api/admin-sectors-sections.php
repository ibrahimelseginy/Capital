...
try {
    ...
    if ($method==='GET') { echo json_encode(['ok'=>true,'data'=>sectors_read()],JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);exit; }
    ...
    $raw=$_POST['content']??null;if (!is_string($raw) || strlen($raw)>180000) throw new InvalidArgumentException('حجم المحتوى غير صالح.');
    ...
    $next=sectors_save($key,$content,$revision,(string)$user['id']);
    echo json_encode(['ok'=>true,'revision'=>$next,'message'=>'تم حفظ القسم ونشره على صفحة القطاعات.'],JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR);
} catch (InvalidArgumentException $error) {
    http_response_code(422);echo json_encode(['ok'=>false,'message'=>$error->getMessage()],JSON_UNESCAPED_UNICODE);
} catch (Throwable $error) {
    error_log('Sectors sections: '.$error->getMessage());$conflict=$error->getCode()===409;http_response_code($conflict?409:503);
